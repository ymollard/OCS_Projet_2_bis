    ...
    for url_categorie in url_categorie_liste:
        ...
        with open(path_data, "w", encoding="utf-8", newline="") as csv_file:
            ...
            for i in range(len(url_livres_liste)):
                writer.writerow([url_livres_liste[i], upcs_liste[i], titre_textes[i], prix_ttc_liste[i], prix_ht_liste[i], stocks_liste[i], description_textes[i], categories_liste[i], avis_liste[i], url_img_liste[i]])